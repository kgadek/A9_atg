...
def playRound(players, dice_count, starting_player):
    ...
    print "PTS = ", points, "   DICES = ", "".join(sorted(dices[0])) + "-" + "".join(sorted(dices[1])) + "-" + "".join(
        sorted(dices[2])) + "-" + "".join(sorted(dices[3])) + " / " + all_dices, "   GAME = ", history

    for i in range(4):
        try:
            players[i].result(points, dices, history)
        except:
            print "EXCEPTION IN RESULT: player %d, perhaps you should fix your `result' function to include the history argument" % (
            i + 1)
            pass
